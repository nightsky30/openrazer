...
#include "razercontroller_driver.h"
 ...
 * Version Information
 */
#define DRIVER_DESC "Razer Controller Device Driver"
    ...
    if(retval == 0) {
        ...
        if(response_report.remaining_packets != request_report->remaining_packets ||
           ...
           response_report.command_id.id != request_report->command_id.id) {
            print_erroneous_report(&response_report, "razercontroller", "Response doesn't match request");
//        } else if (response_report.status == RAZER_CMD_BUSY) {
//            print_erroneous_report(&response_report, "razermouse", "Device is busy");
        } else if (response_report.status == RAZER_CMD_FAILURE) {
            print_erroneous_report(&response_report, "razercontroller", "Command failed");
        } else if (response_report.status == RAZER_CMD_NOT_SUPPORTED) {
            print_erroneous_report(&response_report, "razercontroller", "Command not supported");
        } else if (response_report.status == RAZER_CMD_TIMEOUT) {
            print_erroneous_report(&response_report, "razercontroller", "Command timed out");
        }
    } else {
        print_erroneous_report(&response_report, "razercontroller", "Invalid Report Length");
    }
...
{
    struct razer_controller_device *device = dev_get_drvdata(dev);
...
static ssize_t razer_attr_write_mode_spectrum(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    default:
        printk(KERN_WARNING "razercontroller: Unknown device\n");
        break;
    }
...
static ssize_t razer_attr_write_mode_blinking(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if(count == 3) {
        ...
        printk(KERN_WARNING "razercontroller: Blinking mode only accepts RGB (3byte)\n");
    }
...
static ssize_t razer_attr_write_mode_static(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if(count == 3) {
        ...
        printk(KERN_WARNING "razercontroller: Static mode only accepts RGB (3byte)\n");
    }
...
static ssize_t razer_attr_write_set_key_row(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    while(offset < count) {
        if(offset + 3 > count) {
            printk(KERN_ALERT "razercontroller: Wrong Amount of data provided: Should be ROW_ID, START_COL, STOP_COL, N_RGB\n");
            break;
        }
        ...
        // printk(KERN_ALERT "razercontroller: Row ID: %d, Start: %d, Stop: %d, row length: %d\n", row_id, start_col, stop_col, row_length);

        if(start_col > stop_col) {
            printk(KERN_ALERT "razercontroller: Start column is greater than end column\n");
            break;
        }

        if(offset + row_length > count) {
            printk(KERN_ALERT "razercontroller: Not enough RGB to fill row\n");
            break;
        }
...
static ssize_t razer_attr_write_device_mode(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (count != 2) {
        printk(KERN_WARNING "razercontroller: Device mode only takes 2 bytes.");
    } else {
...
static ssize_t razer_attr_write_set_brightness(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if(count > 0) {
        ...
        printk(KERN_WARNING "razercontroller: Brightness takes an ascii number\n");
    }
...
void razer_controller_init(struct razer_controller_device *dev, struct usb_interface *intf, struct hid_device *hdev)
{
...
{
    struct razer_controller_device *device = hid_get_drvdata(hdev);
    int ret;
...
static int razer_controller_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
    ...
    dev = kzalloc(sizeof(struct razer_controller_device), GFP_KERNEL);
    if(dev == NULL) {
 ...
 */
static struct hid_driver razer_controller_driver = {
    .name = "razercontroller",
    .id_table = razer_devices,
    .probe = razer_controller_probe,
    .remove = razer_controller_disconnect,
    .raw_event = razer_raw_event,
    .input_mapping = razer_input_mapping,
    .input_configured = razer_input_configured
};

module_hid_driver(razer_controller_driver);
